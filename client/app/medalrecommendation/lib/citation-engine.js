...
function generateIndividual(payload, reference) {
  ...
  const integrityChecks = validateIndividualInputIntegrity(
    payload,
    context.limits,
  );
  const data = sanitizeIndividualPayload(payload, context.limits);
  const award =
    context.individualAwards.find((item) => item.name === data.awardName) ||
    null;
  const rosterMap = new Map(
    context.roster.map((item) => [item.dropdownName, item]),
  );
  const recipients = data.recipients
    .map((name) => rosterMap.get(name))
    .filter(Boolean);
...
function generateUnit(payload, reference) {
  ...
  const award =
    context.unitAwards.find((item) => item.name === data.awardName) || null;
  const rosterMap = new Map(
    context.roster.map((item) => [item.dropdownName, item]),
  );
  const recipients = data.recipients
...
function validateIndividualInputIntegrity(payload, limits) {
  ...
  const checks = validateTextLengths([
    [
      ...
      limits.operationName,
      "Operation name",
    ],
    [
      "Citation location length",
      source.location,
      limits.location,
      "Citation location",
    ],
    [
      "Combat element / role length",
      source.role,
      limits.role,
      "Combat element / role",
    ],
    [
      "Medal selection length",
      source.awardName,
      limits.awardName,
      "Medal selection",
    ],
...
function validateUnitInputIntegrity(payload, limits) {
  ...
  const checks = validateTextLengths([
    ...
    [
      "Combat unit name length",
      source.unitName,
      limits.unitName,
      "Combat unit name",
    ],
    [
      "Unit award selection length",
      source.awardName,
      limits.awardName,
      "Unit award selection",
    ],
    [
      "Action character length",
      source.actionCharacter,
...
function buildIndividualSubmission(
  ...
  const ticketTitle = buildIndividualTicketTitle(
    operation,
    award,
    orderedRecipients,
  );
...
function buildUnitSubmission(
  ...
  checks.push(
    check(
      "Combat unit name",
      Boolean(data.unitName),
      "Enter the shared combat-unit name.",
    ),
  );
  checks.push(
    check(
      "Unit award selected",
      Boolean(award),
      "Select a valid operation unit award.",
    ),
  );
  ...
  checks.push(
    check(
      "No duplicate recipients",
      selectedCount >= rules.minimumUnitRecipients &&
        uniqueNames.length === selectedCount,
      "Remove duplicate recipient selections.",
    ),
  );
  checks.push(
  ...
  const namedRecipients = findRecipientMentions(
    data.narrative,
    orderedRecipients,
  );
  checks.push(
...
function sortAwardRecipients(recipients, rankPrecedence) {
  const rankOrder = new Map(
    Array.from(rankPrecedence || []).map((rankAbbreviation, index) => [
      rankAbbreviation,
      index,
    ]),
  );
...
function individualEligibilityWarnings(award) {
  ...
  if (award.leadershipRequired)
    warnings.push("An official leadership position is required.");
...
function grammarAdjustRole(value) {
  ...
  const article =
    vowelStart && !/^(us\b|united\b|one\b)/i.test(original) ? "an" : "a";
...
function recipientSurname(recipient) {
  ...
  if (rosterSurname) {
    const fullNameKey = recipientFullName(recipient)
      .toLowerCase()
      .replace(/[’']/gu, "'");
    const rosterSurnameKey = rosterSurname.toLowerCase().replace(/[’']/gu, "'");
...
function formatIndividualTemplateName(recipients, formatter) {
  const nameFormatter =
    typeof formatter === "function" ? formatter : formalFullName;
...
function normalizeIndividualNarrativeNames(narrative, recipients) {
  ...
  return people.reduce(
    (text, recipient) => {
      ...
      return normalizeRecipientNarrativeNames(
        text,
        recipient,
        includeLastNameOnly,
      );
...
function normalizeRecipientNarrativeNames(
  narrative,
  recipient,
  includeLastNameOnly,
) {
  ...
  return tokenized.replace(new RegExp(token, "g"), () => {
    occurrence += 1;
    return occurrence === 1
      ? formalFullName(recipient)
      : formalShortName(recipient);
  });
}
...
function recipientGivenNames(recipient) {
  ...
  const surnameParts = recipientSurname(recipient)
    .split(/\s+/u)
    .filter(Boolean);
...
function findRecipientMentions(narrative, recipients) {
  ...
  return people
    .filter((recipient) => {
      ...
      if (insensitiveExpression && insensitiveExpression.test(text))
        return true;
