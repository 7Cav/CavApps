"use strict";
    ...
    const SERVICE_TICKET_TITLE_FIELD_KEYS = Object.freeze({
      service_outstanding_volunteer_service_medal: Object.freeze(['nonCombatDepartment']),
      ...
      service_defense_meritorious_service_medal: Object.freeze(['benefitedUnitOrArea']),
      ...
      service_distinguished_service_medal: Object.freeze(['creditedOrganizationalElement']),
    ...
    const CONTROLLED_FIELD_COMPONENTS = Object.freeze([
      'select',
      'month',
      'year',
    ]);
    ...
    function generate(payload, bootstrap) {
      ...
      checks.push(check('Service award selected', Boolean(award), 'Select a valid Service award.'));
      ...
      const fields = validateAndNormalizeFields(source.fields, award, context, checks);
      ...
      const correctedNarrative = normalizeServiceNarrativeNames(narrative, recipients);
    ...
    function appendNarrativeChecks(narrative, award, checks) {
      ...
      checks.push(
        check(
          ...
          Boolean(narrative) && countApproximateSentences(narrative) >= award.minimumSentences,
    ...
    function buildCitation(award, fields, recipients, narrative, template) {
      ...
      const lead =
        award.scope === 'individual' && fullNames ? `${fullNames} ${narrativeLead}` : '';
    ...
    function validateAndNormalizeFields(rawFields, award, context, checks) {
      ...
      (award.fields || []).forEach((field) => {
        ...
        appendFieldChecks(field, rawValue, value, allowedValues, context.limits, checks);
    ...
    function appendFieldChecks(field, rawValue, value, allowedValues, limits, checks) {
      checks.push(
        validateTextLength(
          `${field.label} length`,
          rawValue,
    ...
    function normalizeFieldValue(field, value, allowedValues) {
      ...
      return isControlledField(field) ? findCanonicalValue(value, allowedValues) || value : value;
    }
    ...
    function validateAndResolveRecipients(rawRecipients, award, context, checks) {
      ...
      appendRecipientChecks(selectedNames, normalizedNames, recipients, award, context, checks);

      return sortRecipients(recipients, context.rankPrecedence);
    }

    function normalizeRecipientSelections(rawRecipients) {
      return (Array.isArray(rawRecipients) ? rawRecipients : []).map(cleanText).filter(Boolean);
    }
    ...
    function appendRecipientChecks(
      ...
      checks.push(
        check('Recipient selected', selectedNames.length > 0, 'Select at least one roster member.')
      );
      checks.push(
        check(
      ...
      appendRecipientScopeCheck(selectedNames.length, recipients.length, award, context, checks);
    }

    function appendRecipientScopeCheck(selectedCount, resolvedCount, award, context, checks) {
    ...
    function awardHasServicePeriod(award) {
      ...
      return ['startMonth', 'startYear', 'endMonth', 'endYear'].some((key) => fieldKeys.has(key));
    }
    ...
    function prepareAwardTemplate(award, fields, recipients) {
      ...
      const missingValues = requiredTokens.filter((key) => !hasTemplateValue(values, key));
      ...
      const incompleteTokens = Array.from(new Set([...missingValues, ...unresolvedTokens]));
    ...
    function hasTemplateValue(values, key) {
      return Object.prototype.hasOwnProperty.call(values, key) && Boolean(cleanText(values[key]));
    }
    ...
    function appendSoldiersMedalTemplateValues(values, fields) {
      ...
      values.SOLDIERS_OPENING_PHRASE =
        `multiple, significant and distinguished meritorious ${noun}`;
      values.SOLDIERS_CLOSING_PHRASE = `exceptionally meritorious ${noun}`;
    }

    function appendSuperiorServiceTemplateValues(values, fields) {
      const operations = normalizeLookupValue(fields.leadershipContext) === 'operations';

      values.LEADERSHIP_CONTEXT_PHRASE = operations ? 'operations' : 'a secondary billet';
    ...
    function buildServicePeriod(fields) {
      if (!fields.startMonth || !fields.startYear || !fields.endMonth || !fields.endYear) {
        return '';
      }
      ...
      people.forEach((recipient, index) => {
        ...
        if (includeLastNameOnly) {
          updated = replaceServiceRecipientPatterns(
            updated,
            [withOptionalServiceRecipientInitials(namePattern(recipient.lastName), recipient)]
              .filter(Boolean),
            token,
            'g'
          );
    ...
    function buildServiceRecipientMentionPatterns(recipient) {
      ...
      return Array.from(
        new Set(
          patterns
            .filter(Boolean)
            .map((pattern) => withOptionalServiceRecipientInitials(pattern, recipient))
        )
      ).sort((left, right) => {
    ...
    function caseFlexibleInitialPattern(value) {
      ...
      return lower && lower !== upper ? `[${lower}${upper}]` : escapeRegex(character);
    }
    ...
    function sortRecipients(recipients, rankPrecedence) {
      ...
      return Array.from(recipients || []).sort((left, right) => {
        const leftRank = rankOrder.get(left.rankAbbreviation) ?? Number.MAX_SAFE_INTEGER;
        const rightRank = rankOrder.get(right.rankAbbreviation) ?? Number.MAX_SAFE_INTEGER;
    ...
    function buildTicketTitle(award, fields, recipients) {
      ...
      return ['Medal Recommendation', organization, award.abbreviation, recipientLabel]
        .filter(Boolean)
        .join(' | ');
    }
